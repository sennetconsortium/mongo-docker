...
if [[ "$1" != "dev" && "$1" != "test" && "$1" != "stage" && "$1" != "prod" ]]; then
    echo "Unknown environment '$1', specify 'dev', 'test', 'stage', or 'prod'"
else
    if [[ "$2" != "start" && "$2" != "stop" ]]; then
        ...
        if [ "$2" = "start" ]; then
            ...
            docker-compose -p sennet-mongo-docker-$1 -f docker-compose.$1.yml --compatibility up -d
        elif [ "$2" = "stop" ]; then
            docker-compose -p sennet-mongo-docker-$1 -f docker-compose.$1.yml stop
        fi
    fi
fi
